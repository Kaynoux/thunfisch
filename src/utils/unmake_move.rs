...
//         let moved_piece = self.figures(captured_idx);
...
//                 const ROOK_TO_POS_INVERSE: Bitboard = Bitboard(!(Square(59).to_bit().0));
...
//                 const ROOK_TO_POS_INVERSE: Bitboard = Bitboard(!(Square(61).to_bit().0));
