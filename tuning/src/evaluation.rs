...
pub fn evaluate(board: &Board) -> i32 {
    ...
    if settings::BISHOP_PAIR {
        let (mg_bishop_pair, eg_bishop_pair) = bishop_pair_boni(board);
...
pub fn pawn_structure(board: &Board) -> ([i16; 2], [i16; 2]) {
    // [white, black]
    ...
    for i in 0..=1 {
        ...
        for pawn in board.figure_bb_by_index(i).iter_mut() {
            if settings::PASSED_PAWNS {
                let bonus = passed_pawn_bonus(board, pawn, color);
                mg_pawn_offset[i] += bonus[0];
                eg_pawn_offset[i] += bonus[1];
            }
            if settings::ISOLATED_PAWNS {
                let penalty = isolated_pawn_penalty(board, pawn, color);
...
fn isolated_pawn_penalty(board: &Board, pawn: Bit, friendly: Color) -> [i16; 2] {
...
fn passed_pawn_bonus(board: &Board, pawn: Bit, friendly: Color) -> [i16; 2] {
...
#[allow(clippy::cast_possible_wrap, clippy::cast_possible_truncation)]
pub fn doubled_pawn_penalties(board: &Board) -> [i32; 2] {
...
const fn bishop_pair_boni(board: &Board) -> ([i16; 2], [i16; 2]) {
...
pub fn king_safety(board: &Board, figure_movements: &[Bitboard; 12]) -> ([i16; 2], [i16; 2]) {
